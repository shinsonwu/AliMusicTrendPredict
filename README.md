 ...
 * 这种时间序列问题，主要在与学习出周期性和趋势，为了达到这个目标，可以试用典型的时间序列统计模型如stl分解和arima，这样的模型的弱点是像个黑盒，不太便于添加更多特征；另外一种思路就是基于时间构造特征，比如使用7天前的播放量、30天前的播放量作为特征，这样就体现出了周期性，再使用上k天前的下载量、收藏量、歌手id、歌手歌曲数量、歌手的粉丝数量等，构造一个回归模型，这样便于添加更多特征，模型更加灵活，也更容易使用经典的机器学习回归模型
